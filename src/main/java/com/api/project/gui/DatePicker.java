...
public class DatePicker extends JPanel{
    ...
    private String hourtime = "00";
    private String minutetime = "00";
    private String secondtime = "00";
    ...
    private void commit() {
        if (showDate instanceof JTextField) {
            ((JTextField) showDate).setText(sdf.format(calendar.getTime()).concat(" ").concat(hourtime).concat(":").concat(minutetime).concat(":").concat(secondtime));
    ...
    private class FooterPanel extends JPanel {
        ...
        private void initFooterPanel(){
        	...
        	SpinnerModel hourModel = new SpinnerNumberModel(0, 0, 59, 1);
            ...
            hour.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					...
					hourtime = ((JSpinner)e.getSource()).getValue().toString();
					if(hourtime.trim().length() == 1){
						hourtime = "0".concat(hourtime);
					}
        	...
        	minute.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					// TODO Auto-generated method stub
					minutetime = ((JSpinner)e.getSource()).getValue().toString();
					if(minutetime.trim().length() == 1){
						minutetime = "0".concat(minutetime);
					}
					commit();
				}
			});
        	JSpinner seconds = new JSpinner(secondsModel);
        	seconds.addChangeListener(new ChangeListener() {
				
				@Override
				public void stateChanged(ChangeEvent e) {
					// TODO Auto-generated method stub
					secondtime = ((JSpinner)e.getSource()).getValue().toString();
					if(secondtime.trim().length() == 1){
        	...
        	//ok.setPreferredSize(new Dimension(157, 25));
        	ok.setUI(new BasicButtonUI());  
...
//            });
    ...
    public static void main(String[] args) {
        ...
        DatePicker ser = DatePicker.getInstance();
        javax.swing.JTextField text = new JTextField();
        text.setBounds(10, 10, 200, 30);
        text.setText("2013-10-11 11:12:32");
        ser.register(text);
        
        DatePicker ser2 = DatePicker.getInstance("yyyy年MM月dd日");

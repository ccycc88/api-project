package com.api.project.util.chart;
...
public class ChartUtil {
	...
	private static String NO_DATA_MSG = "未加载到数据";
	...
	public static TimeSeries createTimeseries(String category, Vector<Object[]> dateValues) {
		TimeSeries timeseries = new TimeSeries(category, Millisecond.class);
		...
		if (dateValues != null) {
			...
			for (Object[] objects : dateValues) {
				...
				if (date != null && isNumber(sValue)) {
					...
					timeseries.add(new Millisecond(), dValue);
				}
			}
		}
	...
	public static void setTimeSeriesRender(Plot plot, boolean isShowData, boolean isShapesVisible) {
		...
		//DateTickUnit dateTickUnit = new DateTickUnit(DateTickUnitType.SECOND, 60, new SimpleDateFormat("yyyy-MM")); // 第二个参数是时间轴间距
